...
domaine_etude = st.text_input("Domaine d'étude"), options=["Infra & Cloud", "Transformation Digitale", "Marketing", "Entrepreunariat", "Autre"]
...
note_evaluation_precedente = st.slider("Note évaluation précédente (1-10)", 1, 10, 5)
satisfaction_employee_nature_travail = st.slider("Satisfaction nature travail (1-10)", 1, 10, 5)
satisfaction_employee_equipe = st.slider("Satisfaction équipe (1-10)", 1, 10, 5)
satisfaction_employee_equilibre_pro_perso = st.slider("Satisfaction équilibre pro/perso (1-10)", 1, 10, 5)
note_evaluation_actuelle = st.slider("Note évaluation actuelle (1-10)", 1, 10, 5)
augementation_salaire_precedente = st.selectbox("Augmentation salaire précédente(11-25)", 11, 25, 11)
